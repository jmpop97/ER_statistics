...
for i in characterNum_dic:
    ...
    for mmrBefore in globals()["mmrBefore_datas" + str(i)]:
        for mmrBefore_datas in globals()["mmrBefore_datas" + str(i)][mmrBefore]:
            ...
            globals()["range_num" + str(i)].split_tier(
                mmrBefore,
                (
                    mmrBefore_datas["mmrGain"]
                    + mmr_charges(mmrBefore)
                    - rankpoint
                    - mmrBefore_datas["escapeState"] * 10 / 3
                )
                / killpoint
                * 4,
            )
...
for num in characterNum_dic:
    ...
    for tier_names in tier_range:
        ...
        for ranges in getMMR_range:
            ...
            values += [
                globals()["range_num" + str(num)]
                .tier[tier_name]
                .get(getMMR_range[ranges], 0)
            ]
        per_values = []
        per_courses = []
